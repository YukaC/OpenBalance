...
export function buildMonthSummary(
  ...
  currency?: "ARS" | "USD",
  /**
   * I1 light: optional prefiltered txs for this month.
   * When provided, skips re-scanning the full ledger for weeks + category breakdown.
   */
  prefilteredMonthTransactions?: Transaction[],
  /**
   * monthly → calendar-month totals (weeks remain secondary metadata).
   * weekly → pay-week totals. Default weekly for callers that omit it.
   */
...
export function findBudgetAlerts(
  ...
  return progress
    .filter(
      (row): row is typeof row & { level: BudgetAlertLevel } =>
        row.level !== "ok",
    )
    .map((row) => ({
      budget: row.budget,
